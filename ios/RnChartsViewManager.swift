...
class RnChartsView : UIView {
    ...
    @objc
    func setAddCandles(_ candles: NSArray) {
        var index: Int = chartView.data?.entryCount ?? 0
        for item in data {
            chartView.data?.appendEntry(createCandlestick(index: index, item: item), toDataSet: 0)
            index += 1
        }
